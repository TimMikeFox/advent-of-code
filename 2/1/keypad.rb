...
# right. Each line of instructions corresponds to one button, starting
# at the previous button (or, for the first line, the "5" button); press
...
# can't, and stay on "1"), so the first button is 1 Starting from the    .
# previous button ("1"), you move right twice (to "3") and then down     .
# three times (stopping at "9" after two moves and ignoring the third),  .
# ending up with 9 Continuing from "9", you move left, up, right, down,  .
# and left, ending with 8 Finally, you move up four times (stopping at   .
# "2"), then down once, ending with 5 So, in this example, the bathroom  .
# code is 1985                                                           .
# 
...
STDIN.each_line do |line|
  line.each_char do |c|
    case c
    when 'U'
      position[1] = position[1] - 1 unless position[1] == 0
    when 'D'
      position[1] = position[1] + 1 unless position[1] == 2
    when 'L'
      position[0] = position[0] - 1 unless position[0] == 0
    when 'R'
      position[0] = position[0] + 1 unless position[0] == 2
    end
